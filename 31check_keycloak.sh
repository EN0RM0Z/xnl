...
# Все 3 проверки должны быть успешными
TOKEN_MIN_SUCCESS=3
...
# Пауза между отдельными проверками, секунд
CHECK_DELAY=1

# Пауза между повторными уведомлениями при длительной аварии,
# минут
NOTIFY_INTERVAL=30
...
# User-Agent нашего мониторинга.
#
# Позволяет найти запросы мониторинга в Nginx access.log.
USER_AGENT="Keycloak-Monitor/1.0 (check-script)"


# ============================================================
# АВТОМАТИЧЕСКИЙ ПЕРЕЗАПУСК KEYCLOAK
# ============================================================

# 1 - включить автоматический перезапуск
# 0 - отключить автоматический перезапуск

RESTART_KEYCLOAK=1

# Путь к существующему скрипту перезапуска

RESTART_SCRIPT="/opt/keycloak/scripts/restartkeyc.sh"
...
if [[ "$SITE_AVAILABLE" -eq 1 ]]; then
    ...
    # --------------------------------------------------------
    # ЕСЛИ ДО ЭТОГО БЫЛ DOWN —
    # ЗНАЧИТ САЙТ ВОССТАНОВИЛСЯ
    # --------------------------------------------------------
...
# ============================================================
    ...
    if [[ "$SITE_STATE" != "DOWN" ]]; then
        ...
        # АВТОМАТИЧЕСКИЙ ПЕРЕЗАПУСК KEYCLOAK
        #
        # Выполняется ТОЛЬКО при первом обнаружении DOWN.
        #
        # Символ & означает:
        # не ждать окончания restartkeyc.sh,
        # а продолжить выполнение мониторинга.
        # ====================================================

        if [[ "$RESTART_KEYCLOAK" -eq 1 ]]; then

            if [[ -x "$RESTART_SCRIPT" ]]; then

                "$RESTART_SCRIPT" &

                LOG_MESSAGE="${LOG_MESSAGE}, ПЕРЕЗАПУСК KEYCLOAK: ЗАПУЩЕН"

            else

                LOG_MESSAGE="${LOG_MESSAGE}, ПЕРЕЗАПУСК KEYCLOAK: СКРИПТ НЕ НАЙДЕН"
